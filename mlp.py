...
"""
# from theano.sandbox.symbolic_module import logistic_regression
    ...
    def __init__(self, rng, inputs, n_in, n_out, W=None, b=None,
        ...
        if W is None:
            ...
            if activation == theano.tensor.nnet.sigmoid:
                W_values *= 4
            # W = W_values
            self.W = theano.shared(value=W_values, name='W_hid', borrow=True)
        else:
            self.W = theano.shared(
                    numpy.asarray(W, dtype=theano.config.floatX),
                    name='W_hid',
                    borrow=True
            )
        if b is None:
            b_values = numpy.zeros((n_out,), dtype=theano.config.floatX)
            self.b = theano.shared(value=b_values, name='b_hid', borrow=True)
            # b = b_values
        else:
            self.b = theano.shared(
                    numpy.asarray(b, dtype=theano.config.floatX),
                    name='b_hid',
                    borrow=True
            )

        lin_output = T.dot(inputs, self.W) + self.b
        # here we allow None for activation function
        self.output = (
